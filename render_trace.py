...
def human_block(block, corrections) -> str:
    ...
    if not pairs:
        return md_body(block["lines"])
    raw = "\n".join(block["lines"])
    return md_body(apply_corrections(raw, pairs).splitlines())
...
def render(sources, corrections=None) -> str:
    ...
    links = " &middot; ".join(
        ...
        for p in parts)
    lede = (
        'Every session, in order, nothing removed and nothing reordered. Two things are '
        'presentation rather than record: tool results arrive under the same "user" role as the '
        'person, so they are labelled apart and folded into lists, and typos in the human&rsquo;s own '
        'messages are corrected from '
        '<a href="{blob}trace-corrections.json">an explicit list of find/replace pairs</a>. '
        'The files of record are verbatim and unedited: {links}.'
    ).format(blob=BLOB, links=links)
    foot = (
...
def self_test() -> int:
    ...
    if "welcome!" not in corrected:
        failures.append("a correction did not reach the page")
    if "Hello" not in corrected:
        failures.append("correcting a message dropped the rest of it")
...
def corrected_source(markdown_text: str, corrections) -> str:
    """The source as the page renders it: same text, typos fixed in place.

    Without this the check quietly weakens the moment a correction lands - the
    original spelling still appears elsewhere on the page, quoted inside a later
    message, so a naive search finds it and reports success about the wrong
    occurrence. Comparing against what the page is supposed to say keeps the
    guarantee honest.
    """
    if not corrections:
    ...
    for line in markdown_text.splitlines():
        out.append(line)
    text = "\n".join(out)
    for pairs in corrections.values():
        for find, replace in pairs:
...
def main(argv) -> int:
    ...
    if args.verify:
        missing = []
        for text, name in loaded_sources:
            checked = corrected_source(text, corrections.get(name))
            missing.extend((name, line) for line in verify(checked, page))
        if missing:
