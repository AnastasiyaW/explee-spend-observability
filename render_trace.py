...
def render(sources, corrections=None) -> str:
    ...
    links = " &middot; ".join(
        ...
        for p in parts)
    lede = (
        'Every session, in order, nothing removed and nothing reordered. One thing here is '
        'presentation rather than record: tool results arrive under the same "user" role as the '
        'person, so they are labelled apart and folded into lists. The files of record: {links}.'
    ).format(links=links)
    foot = (
